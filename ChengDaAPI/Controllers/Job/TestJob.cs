...
namespace ChengDaApi.Controllers.Job
{
    ...
    public class TestJob : IJob
    {
        ...
        private int count = 0;
        public TestJob(ILogger<TestJob> logger, IServiceProvider provider)
        {
        ...
        public Task Execute(IJobExecutionContext context)
        {
            ...
            Console.WriteLine($"@{DateTime.Now:HH:mm:ss} - job{jobName} - start");
            for (int i = 0; i < 5; i++)
            {
                ...
                Console.WriteLine($"@{DateTime.Now:HH:mm:ss} - job{jobName} - working{i}");

            }

            count++;
            Console.WriteLine($"TestJob Count => {count}");
            Console.WriteLine($"@{DateTime.Now:HH:mm:ss} - job{jobName} - done");
